...
- **BLE и WiFi работают одновременно** — BLE как основной источник, Nightscout по WiFi как fallback: показания NS применяются, только если они новее уже имеющихся, поэтому WiFi не перебивает свежие данные из приложения
- WiFi настойчиво переподключается в фоне (watchdog каждые 30 сек) и может служить единственным источником данных, если BLE не используется
- NTP синхронизация времени при наличии WiFi, с повторной попыткой в фоне при позднем/повторном подключении
- BLE-пароль сохраняется в NVS — не перегенерируется при каждой перезагрузке, повторное сопряжение с телефоном не требуется
- BLE-стек — NimBLE-Arduino (не штатный Bluedroid): заметно меньше занимает памяти, что даёт BLE и WiFi/HTTPS работать одновременно без танцев с выключением BLE
- Аппаратный watchdog — если что-то всё-таки зависнет, устройство перезагрузится само в течение 30 секунд, без необходимости выдёргивать питание
...
- **WiFi SSID задан** → точка доступа выключается, устройство остаётся в режиме STA (переподключается в фоне, даже если ещё не успело подключиться при старте), веб-интерфейс доступен по локальному IP
...
Устройство представляется как `CYDDrip`.
