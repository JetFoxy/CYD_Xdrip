...
struct tConfig {
  ...
  char  blepassword[64]   = {0};   // BLE auth password; empty = auto-generate
  int   utc_offset_min    = 0;     // UTC offset in minutes (fallback if BLE doesn't send tz)
  float bg_low            = 3.9f;  // BG threshold: urgent low
  ...
  char  ntp_server[64]    = {0};   // NTP server hostname; empty = pool.ntp.org
};
